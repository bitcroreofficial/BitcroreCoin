...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("Bitcrore_CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("Bitcrore_CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("Bitcrore_CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("bitcrore_coin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Bitcrore_CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("Bitcrore_CoinMiner exception");
    } catch (...) {
        LogPrintf("Bitcrore_CoinMiner exception");
    }

    LogPrintf("Bitcrore_CoinMiner exiting\n");
}
