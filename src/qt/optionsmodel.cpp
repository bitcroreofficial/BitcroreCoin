...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BC);
    ...
    if (settings.contains("nAnonymizeBitcrore_CoinAmount") || reset)
        SoftSetArg("-anonymizebitcrore_coinamount", settings.value("nAnonymizeBitcrore_CoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitcrore_coin.cpp during shotdown to also remove the window positions
