...
#include "qt/bitcrore_coin/dashboardwidget.h"
#include "qt/bitcrore_coin/forms/ui_dashboardwidget.h"
#include "qt/bitcrore_coin/sendconfirmdialog.h"
#include "qt/bitcrore_coin/txrow.h"
...
DashboardWidget::DashboardWidget(Bitcrore_CoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareBc, "square-chart-bc");
    setCssProperty(ui->labelBc, "text-chart-bc");
    ...
    setCssProperty(ui->labelAmountBc, "text-stake-bc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &Bitcrore_CoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &Bitcrore_CoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &Bitcrore_CoinGUI::openFAQ);
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(11, 11, 20);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            bc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalBc += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBc.append(bc);

        int max = std::max(bc, bc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(55, 80, 110));
    ...
    set0->append(chartData->valuesBc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBc > 0) {
        setCssProperty(ui->labelAmountBc, "text-stake-bc");
    } else {
        setCssProperty(ui->labelAmountBc, "text-stake-bc-disable");
    }
    forceUpdateStyle({ui->labelAmountBc});
    ui->labelAmountBc->setText(GUIUtil::formatBalance(chartData->totalBc, nDisplayUnit));
