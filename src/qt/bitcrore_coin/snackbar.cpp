...
#include "qt/bitcrore_coin/forms/ui_snackbar.h"
#include "qt/bitcrore_coin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(Bitcrore_CoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &Bitcrore_CoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
