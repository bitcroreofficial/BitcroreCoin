...
#if defined(NDEBUG)
#error "Bitcrore_Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubc) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateBCSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating BC supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating BC supply..."), percent);
		}
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight <= last_pow_block && nHeight >= 1) {
		nSubsidy = 75000 * COIN;
    } else if (nHeight <= 52566681 && nHeight > last_pow_block) {
        nSubsidy = 1.617 * COIN;
    } else if (nHeight == 52566682) {
        nSubsidy = 0.223 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
{
    util::ThreadRename("bitcrore_coin-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> bcInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                bcInputs.push_back(stakeIn);
        }
        const bool hasBCInputs = !bcInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBCInputs) {
                    ...
                    for (const CTxIn& bcIn : bcInputs)
                        if(bcIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasBCInputs) {
                            for (const CTxIn& stakeIn : bcInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // Bitcrore_Coin: We use certain sporks during IBD, so check to see if they are
