...
- [Developer Notes](#developer-notes)
    ...
    - [Development tips and tricks](#development-tips-and-tricks)
        ...
        - [Vbcrind suppressions file](#vbcrind-suppressions-file)
...
- [Development guidelines](#development-guidelines)
    - [General Bitcrore Coin](#general-bitcrore_coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bitcrore Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play BCs (tBC)" on the test network, if you
...
Bitcrore Coin is a multi-threaded application, and deadlocks or other
multi-threading bugs can be very difficult to track down. The `--enable-debug`
configure option adds `-DDEBUG_LOCKORDER` to the compiler flags. This inserts
run-time checks to keep track of which locks are held, and adds warnings to the
debug.log file if inconsistencies are detected.

### Vbcrind suppressions file

Vbcrind is a programming tool for memory debugging, memory leak detection, and
profiling. The repo contains a Vbcrind suppressions file
([`vbcrind.supp`](https://github.com/bitcrore_coin-project/bitcrore_coin/blob/master/contrib/vbcrind.supp))
which includes known Vbcrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ vbcrind --suppressions=contrib/vbcrind.supp src/test/test_bitcrore_coin
$ vbcrind --suppressions=contrib/vbcrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitcrore_coin --log_level=test_suite
$ vbcrind -v --leak-check=full src/bitcrore_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bitcrore_coin.coverage/index.html`.
```
...
- BitcoinMiner : Generates BCs (if wallet is enabled).
...
However, in open source software such as Bitcrore Coin, where everyone uses
...
pay attention to for reviewers of Bitcrore Coin code.

General Bitcrore Coin
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Bitcrore Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Bitcrore Coin so that they can be integrated
...
file descriptors it will cause problems with Bitcrore_Coin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Bitcrore_Coin-Project/Bitcrore_Coin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitcrore_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitcrore_coin-cli` and the GUI debug console use this table to determine how to
